...
def get_available_bookings(mysqlcon, batch_size):
    with mysqlcon.cursor() as cursor:
        sql = "SELECT * FROM `dme_bookings` WHERE `b_status`=%s and LOWER(`vx_freight_provider`)=%s and z_label_url is null ORDER BY `id` DESC LIMIT %s"
        cursor.execute(sql, ('Booked','startrack', batch_size))
...
def get_label(booking, mysqlcon):
    ...
    try:
        ...
        try:
            success_booking_id = data0[0]['Created label url']
