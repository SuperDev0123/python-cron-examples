...
class CopeAuMoveLabelFilesResource(ModelResource):
    class Meta:
        ...
        try:
            ...
            #source = "dme_sftp/cope_au/labels/indata/"
            #dest = "dme_api/static/pdfs/"
            ...
            for file in glob.glob(os.path.join(source,"*.pdf")):                      
                print(ntpath.basename(file).split('_')[1]) 
                print(ntpath.basename(file).split('_')[2].split('.')[0])  
                shutil.copy2(file, dest+'TEST_'+ntpath.basename(file))
                sql2 = "UPDATE dme_bookings set z_label_url = %s, z_downloaded_shipping_label_timestamp = %s WHERE b_bookingID_Visual = %s"
                adr2 = ('TEST_'+ntpath.basename(file), time.time(), ntpath.basename(file).split('_')[2].split('.')[0], )
