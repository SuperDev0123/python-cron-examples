...
def do_process(fpath, mysqlcon):
    with open(fpath) as csv_file:
        for i, line in enumerate(csv_file):
            ...
            fp_store_event_date = datetime.datetime.strptime(line.split(',')[2], '%Y-%m-%d')
            fp_store_event_time = datetime.datetime.strptime(line.split(',')[3].replace('\n', ''), '%H:%M')

            date = None
            if not 'o' in line.split(',')[1]:
                ...
                with mysqlcon.cursor() as cursor:
                    if date:
                        sql = "UPDATE `dme_bookings` \
                               ...
                               WHERE id=%s"
                        cursor.execute(sql, (date, date, fp_store_event_date, fp_store_event_time, booking['id']))
                    else:
                        sql = "UPDATE `dme_bookings` \
                               SET fp_store_event_date=%s, fp_store_event_time=%s \
                               WHERE id=%s"
                        cursor.execute(sql, (fp_store_event_date, fp_store_event_time, booking['id']))
                    mysqlcon.commit()
            else:
