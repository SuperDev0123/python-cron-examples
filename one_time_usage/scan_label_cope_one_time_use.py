...
def get_booking(dme_number, mysqlcon):
    with mysqlcon.cursor() as cursor:
        sql = "SELECT `id`, `pk_booking_id`, `e_qty_scanned_fp_total`, `delivery_kpi_days`, `dme_status_detail`, \
                ...
                From `dme_bookings` \
                WHERE `v_FPBookingNumber`=%s AND z_CreatedTimestamp > %s \
                AND (e_qty_scanned_fp_total=%s OR e_qty_scanned_fp_total is NULL)"
        cursor.execute(sql, (dme_number, "2020-01-01 00:25:20", 0))
...
def do_calc_scanned(dme_number, dme_number_lines, mysqlcon):
    if dme_number != "DME134729":
        return

    print("@1 - ", dme_number, dme_number_lines)
