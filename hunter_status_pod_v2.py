            ...
            if con_index == 0:
                ...
                for index, line in enumerate(content):
                    ...
                    if (booking 
                        ...
                        requests.post(f"{API_URL}/statushistory/save_status_history/", headers=headers, data=json.dumps({
...
if __name__ == "__main__":
    ...
    if not path.isdir(INPROGRESS_DIR) or not path.isdir(ARCHIVE_DIR):
        print('Given argument "%s, %s" is not a directory' % INPROGRESS_DIR, ARCHIVE_DIR)
        exit(1)
