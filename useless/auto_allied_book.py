...
def get_available_bookings(mysqlcon, batch_size):
    with mysqlcon.cursor() as cursor:
        sql = "SELECT * FROM `dme_bookings` WHERE `b_status`=%s and LOWER(`vx_freight_provider`)=%s ORDER BY `id` ASC LIMIT %s"
        cursor.execute(sql, ('Ready for booking', 'allied', batch_size))
...
def book(booking, mysqlcon):
    url = "http://ec2-35-162-92-135.us-west-2.compute.amazonaws.com/api/booking_allied/"
    ...
    try:
        error = data0[0]['Error']
        print('#100, Book failed:', booking['b_bookingID_Visual'], error)
        save_error(booking['id'], error, mysqlcon)
    except KeyError:
        try:
            success_booking_id = data0[0]['Created Booking ID']
            print('#101, Book success Consignment Number: ', success_booking_id,
                  " Booking Visual id : ", booking['b_bookingID_Visual'])
            return True
        except KeyError:
...
def get_label(booking, mysqlcon):
    ...
    try:
        error = data0[0]['Error']
        print('#100, Get_Label failed:', booking['b_bookingID_Visual'], error)
        save_error(booking['id'], error, mysqlcon)
    except KeyError:
        try:
            success_booking_id = data0[0]['Created Booking ID']
...
if __name__ == '__main__':
    ...
    bookings = get_available_bookings(mysqlcon, 10)
