...
def get_available_bookings(mysqlcon, batch_size):
    with mysqlcon.cursor() as cursor:
        sql = "SELECT * FROM `dme_bookings` WHERE `b_status`=%s and LOWER(`vx_freight_provider`)=%s ORDER BY `id` DESC LIMIT %s"
        cursor.execute(sql, ('Ready for booking','startrack', batch_size))
