...
def do_import(dbcon, cur, filename):
    ...
    while True:
        ...
        header["fk_client_id"] = "d69f550a-9327-4ff9-bc8f-242dfca00f7e"
...
def download_from_sharepoint(dbcon):
    ...
    folderpath = "Shared Documents/Bookings/Tempo_Big_W/1_Drop"
    completed_path = f"/sites/DeliverMECustomerSite/Shared Documents/Bookings/Tempo_Big_W/3_Completed"
    error_path = f"/sites/DeliverMECustomerSite/Shared Documents/Bookings/Tempo_Big_W/4_ErrorToCheck"
    DEV_SUFFIX = "_Test"
...
if __name__ == "__main__":
    ...
    try:
        option = get_option(dbcon, "xls_import_tempo_big_w")

        if int(option["option_value"]) == 0:
            print("#905 - `xls_import_tempo_big_w` option is OFF")
        elif option["is_running"]:
            print("#905 - `xls_import_tempo_big_w` script is already RUNNING")
        else:
            print("#906 - `xls_import_tempo_big_w` option is ON")
            set_option(dbcon, "xls_import_tempo_big_w", True)
            print("#910 - Processing...")
            download_from_sharepoint(dbcon)
            set_option(dbcon, "xls_import_tempo_big_w", False, time1)
    except Exception as e:
